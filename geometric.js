...
function ViewQuery()
{
	...
	ajaxRequest.open("POST", "getPdbFragment.php", true);
...
function LookUpNTs(nts,mode)
{
	...
	if (nts.value != '') {
		...
		if (mode == 'asynch') {
    		ajaxRequest.open("POST", "parseNTs.php", true);
    		ajaxRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    		ajaxRequest.send(queryString);
    		ajaxRequest.onreadystatechange = function() {//Call a function when the state changes.
    			if(ajaxRequest.readyState == 4 && ajaxRequest.status == 200) {
    		...
    		ajaxRequest.open("POST", "parseNTs.php", false);
    		ajaxRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    		ajaxRequest.send(queryString);
            return ReactToAjaxResponse(nts,ajaxRequest.responseText);
		}
...
//			document.getElementById('matrix').removeNode(chaintr);
	...
	if (mode == 'asynch') {
		ajaxRequest.open("GET", "lookupchains.php" + queryString, true);
   		ajaxRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
   		ajaxRequest.send(queryString);
   		ajaxRequest.onreadystatechange = function() {//Call a function when the state changes.
   			if(ajaxRequest.readyState == 4 && ajaxRequest.status == 200) {
		...
		ajaxRequest.open("GET", "lookupchains.php" + queryString, false);
   		ajaxRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
   		ajaxRequest.send(queryString);
