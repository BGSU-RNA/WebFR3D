...
function reportMistake(result,id,message,Query)
    ...
    fprintf(fid, '<html><head><link rel="stylesheet" type="text/css" href="%s/Library.css"><title>WebFR3D results</title></head><body>',config.css);
    ...
    if isfield(Query, 'Email')
        link = sprintf('%s/%s', config.web_results, id);
        ...
        command = sprintf('echo "%s" | tee email.txt | mail -s "WebFR3D results %s" %s; rm email.txt;', message, id, Query.Email);
        unix(command);
        clear Query.Email;
    end
    disp('Standby');
