...
for f=1:length(Filenames),
  ...
  if (ReadPDB == 1) && (isempty(strfind(PDBFilename,'.pdb1'))) && length(File.NT) > 0,
    ...
    if (r < 0.4),                                 % very few basepairs found
      if Verbose > 0,
        fprintf('Few basepairs found (%7.2f basepairs per nucleotide), reading the biological unit coordinates\n',r);
      end
      ...
      if length(File1.NT) > 0,
        ...
        if Verbose > 0,
          fprintf('Biological unit coordinates have %7.2f basepairs per nucleotide\n',r1);
        end
        if r1 > r,
